...
import { Search, Filter, Calendar, Star, Verified, ArrowRight, MessageSquare, Briefcase, Sparkles, MapPin, ExternalLink } from 'lucide-react';
    ...
    <div className="min-h-full bg-background grid-pattern p-8 lg:p-16 overflow-y-auto">
      ...
      <section className="max-w-7xl mx-auto mb-20 flex flex-col md:flex-row md:items-end justify-between gap-12">
        <div className="max-w-3xl">
          <Badge variant="primary" className="mb-6">Global Expert Network</Badge>
          <Heading level={1} className="mb-6 leading-tight">Legal Marketplace</Heading>
          <Text className="text-xl">
            Connect with specialized legal consultants for verified analysis and direct advisory sessions. Architecting trust through verified expertise.
          </Text>
        </div>
        <div className="flex gap-4">
          <div className="px-8 py-5 bg-white border border-surface-container-high rounded-2xl flex items-center gap-4 shadow-xl shadow-primary/5">
            ...
            <div>
              <Text variant="label" className="mb-0.5">Expert Status</Text>
              <div className="text-lg font-black text-on-surface tracking-tighter">Verified Pros Only</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto mb-16 flex flex-wrap items-center gap-8">
        <div className="flex-1 min-w-[400px]">
          <div className="relative group">
            <Search className="absolute left-6 top-1/2 -translate-y-1/2 text-on-surface-variant group-focus-within:text-primary transition-all group-focus-within:scale-110" size={24} />
            <input 
      ...
      <section className="max-w-7xl mx-auto mt-32">
        ...
        >
          ...
          <div className="lg:w-1/2 relative z-10">
            ...
            <Heading level={2} className="mb-10 leading-tight">Recommended for your recent Analysis</Heading>
            <Text className="text-xl mb-12 font-medium leading-relaxed">
              Based on your <span className="text-primary font-black">Master_Service_Agreement_v4.pdf</span>, Dr. Aris Thorne specializes in the cross-border regulatory hurdles identified.
            </Text>
          ...
          <div className="lg:w-1/2 w-full grid grid-cols-2 gap-8 relative z-10">
            <MetricCard icon={<Verified className="text-primary" />} label="Tier 1 Expert" value="98%" sub="Relevance Match" />
            <MetricCard icon={<Calendar className="text-primary" />} label="Response" value="< 2hrs" sub="Turnaround" />
            <div className="bg-white/80 backdrop-blur-xl p-10 rounded-[2.5rem] shadow-xl shadow-primary/5 border border-surface-container-high col-span-2 flex items-center gap-8 group cursor-pointer hover:border-primary/20 transition-all">
              ...
              <div>
                <div className="text-2xl font-black text-on-surface mb-1">Dr. Aris Thorne</div>
              ...
              <ArrowRight className="ml-auto text-primary/20 group-hover:text-primary group-hover:translate-x-2 transition-all" size={24} />
            </div>
          </div>
        </motion.div>
      </section>
...
const FilterButton = ({ icon, label }) => (
  <button className="flex items-center gap-4 px-8 py-5 bg-white hover:bg-surface-container-low rounded-[1.5rem] transition-all border-2 border-surface-container-high hover:border-primary/20 shadow-sm group">
    ...
    <span className="text-sm font-black text-on-surface uppercase tracking-widest">{label}</span>
  </button>
);

const LawyerCard = ({ name, title, rating, reviews, price, status, image, location }) => (
  <motion.div 
    whileHover={{ y: -12 }}
    ...
    <div className="p-12 flex flex-col h-full">
      <div className="flex items-start justify-between mb-10">
        <div className="relative">
          ...
          />
          <div className={`absolute -right-1 -bottom-1 w-6 h-6 border-4 border-white rounded-full relative z-20 ${status === 'online' ? 'bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.5)]' : 'bg-amber-400'}`}></div>
        </div>
        <div className="text-right">
          <div className="flex items-center justify-end gap-2 text-primary">
            <Star size={20} fill="currentColor" />
            <span className="font-black text-2xl tracking-tighter">{rating}</span>
          </div>
          <div className="text-[10px] uppercase tracking-[0.2em] text-on-surface-variant font-black mt-2">{reviews} Reviews</div>
        </div>
      </div>
      
      <Heading level={3} className="mb-2 leading-tight group-hover:text-primary transition-colors">{name}</Heading>
      <Text variant="detail" className="font-bold mb-4 opacity-80">{title}</Text>
      ...
      <div className="mt-auto pt-10 border-t border-surface-container-high flex items-center justify-between">
        <div>
          <Text variant="label" className="mb-1 opacity-50">Consultation</Text>
          <div className="flex items-baseline gap-1.5">
            <span className="text-3xl font-black text-on-surface tracking-tighter">${price}</span>
            <span className="text-[10px] font-black text-on-surface-variant uppercase tracking-[0.2em]">/ session</span>
          </div>
        </div>
        <button className="w-16 h-16 bg-surface-container-low text-primary rounded-[1.5rem] hover:bg-primary hover:text-white hover:shadow-2xl hover:shadow-primary/40 transition-all active:scale-95 flex items-center justify-center group/btn border border-surface-container-high/50">
          <ArrowRight size={24} className="transition-transform group-hover/btn:translate-x-1" />
        </button>
      </div>
    </div>
  </motion.div>
...
const MetricCard = ({ icon, label, value, sub }) => (
  <Card padding="p-8" className="shadow-xl shadow-primary/5 flex flex-col gap-6 group hover:border-primary/20 transition-all">
    <div className="flex items-center gap-4">
      <div className="p-3 bg-primary/5 rounded-2xl text-primary transition-colors group-hover:bg-primary group-hover:text-white">
        {icon}
      </div>
      <Text variant="label" className="group-hover:text-primary transition-colors">{label}</Text>
    </div>
    <div>
      <div className="text-4xl font-black text-on-surface tracking-tighter mb-1">{value}</div>
      <Text variant="detail" className="opacity-60">{sub}</Text>
    </div>
  </Card>
);
